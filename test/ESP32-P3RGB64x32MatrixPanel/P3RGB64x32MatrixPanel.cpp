...
esp_timer_handle_t esp32s3_timer = nullptr;
void IRAM_ATTR timer_callback(void* arg) {
  P3RGB64x32MatrixPanel::onTimer();
}
...
void P3RGB64x32MatrixPanel::begin() {
  ...
  esp_timer_create_args_t timer_args = {
    ...
    .arg = nullptr,
    // .dispatch_method = ESP_TIMER_TASK, // Có thể bỏ dòng này
    .name = "matrix_timer"
  };
  esp_timer_create(&timer_args, &esp32s3_timer);
  esp_timer_start_periodic(esp32s3_timer, 30); // 30us period
#else
...
#if CONFIG_IDF_TARGET_ESP32S3
  if (esp32s3_timer) {
    esp_timer_stop(esp32s3_timer);
    esp_timer_delete(esp32s3_timer);
    esp32s3_timer = nullptr;
  }
#else
  if (timer) {
